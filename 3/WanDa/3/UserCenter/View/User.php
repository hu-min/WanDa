...
class User {
	 ...
	 * @param unknown $user        	
	 */
	public function JieXiUser0($user) {
		$obj = json_decode ( $user );
		$arry = $obj->{'userlist'};
		switch ($arry [0]->{'gender'}) {
			case 0 :
				$gender = "未设置";
				break;
			case 1 :
	 ...
	 * @return string
	 */
	public function JieXiUser($user) {
		$obj = json_decode ( $user );
		$arry = $obj->{'userlist'};
		switch ($arry [0]->{'gender'}) {
			case 0 :
	...
	public function getUserByName($name, $fuzzy = TRUE) {
		...
		// 如果传入无名字，返回错误，但是返回值需要统一（和微信服务器返回的一致json），
		// 所以我们需要打包json_encode函数成json
		if (empty ( $name )) {
			return json_encode ( array (
					'success' => FALSE,
	...
	public function createUser($userid, $name, $departmentid, $mobile = FALSE, $email = FALSE, $weixinid = FALSE, $position = FALSE, $gender = FALSE, $avatar_mediaid = FALSE, $extattr = array()) {
		if (empty ( $userid ) || empty ( $name ) || empty ( $departmentid )) {
			return json_encode ( array (
					'success' => FALSE,
					'errmsg' => 'Userid or name or departmentid is empty!',
					'errcode' => - 2 
			) );
		}
		...
		if (! empty ( $extattr ) && is_array ( $extattr )) {
			...
			$data ['extattr'] = $this->tools->json_encode_ex ( array (
					'attrs' => $temp 
			) );
		}
	 ...
	 *        	形如["zhangsan", "lisi"]的一维数组
	 */
	public function batchDeleteUsers($useridlist) {
		if (empty ( $useridlist )) {
			return json_encode ( array (
					'success' => FALSE,
					'errmsg' => 'Useridlist is empty!',
					'errcode' => - 2 
			) );
		} else if (! is_array ( $useridlist )) {
			return json_encode ( array (
					'success' => FALSE,
					'errmsg' => 'Useridlist not an array!',
					'errcode' => - 2 
			) );
		}
		...
		if ($result) {
			...
			return json_encode ( array (
					'success' => FALSE,
					'errmsg' => 'Batch delete users fails!',
					'errcode' => - 2 
			) );
		}
	}
