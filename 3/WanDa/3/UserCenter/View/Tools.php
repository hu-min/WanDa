...
class Tools {
	...
	private function httpGetRequest($url, $parameters = NULL) {
		...
		// 将请求参数追加在url后面
		// is_array:若变量为数组类型则返回 true，否则返回 false。
		// count:当变量未被设置，或是变量包含一个空的数组，该函数会返回 0。
	...
	public function getAccessToken($corpid, $corpsecret) {
		...
		$result = json_decode ( $result, TRUE );
		// 字符串拼接。。吧$corpid和$corpsecret拼接起来，
		// 例如$corpid=“123”；$corpsecret=“456”；
		// 那么$key = $corpid . $corpsecret;结果是$key=“123456”
		// token.bin文件存放形式，json形式的键值对
		// {
		
		// "corpid . $corpsecret":time：txxxxxxxxxxxxxxxxxx,
		// "corpid . $corpsecret":time：txxxxxxxxxxxxxxxxxx,
		// "corpid . $corpsecret":time:txxxxxxxxxxxxxxxxxx,
		// "corpid . $corpsecret":time:txxxxxxxxxxxxxxxxxx,
		// "corpid . $corpsecret":time:txxxxxxxxxxxxxxxxxx,
	...
	public function json_encode_ex($value) {
		if (version_compare ( PHP_VERSION, '5.4.0', '<' )) {
			...
			$str = preg_replace_callback ( "#\\\u([0-9a-f]{4})#i",matchs($matchs) , $str );
			return $str;
		} else {
